...
def main():
    ...
    if args.evaluate:
        ...
        if args.local_rank == 0:
            ...
            # FGSM attack
            adv_test_loss, adv_test_acc = test(testloader, model, criterion, epsilon=epsilon, fgsm=True)
            print(' FGSM Adv Test Loss:  %.8f, Adv Test Acc:  %.2f' % (adv_test_loss, adv_test_acc))
            logger.append([0.0, 0.0, adv_test_loss, 0.0, adv_test_acc])
            logger.close()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % 5 == 4: 
            train_loss, train_acc = train_stdp(trainloader, model, criterion, optimizer, warmup=args.warmup)
        else:
...
def train_stdp(trainloader, model, criterion, optimizer, warmup=0):
    # switch to train mode
    model.train()
    cnt = 0
    stdp_list = []
    for layers in model.modules():
        if isinstance(layers, nn.Sequential):
            for i, layer in enumerate(layers):
                if isinstance(layer, nn.Conv2d) and cnt < 5:
...
def pgd_attack(model, criterion, images, labels, epsilon, alpha, num_iter):
    ...
    for _ in range(num_iter):
        ...
        # projection
        perturbed = torch.max(torch.min(perturbed, original_images + epsilon), original_images - epsilon)
        perturbed = torch.clamp(perturbed, 0, 1).detach().requires_grad_(True)

    return perturbed.detach()

def test(testloader, model, criterion, epsilon=0.0, fgsm=False):
    global best_acc
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        inputs, targets = inputs.cuda(), targets.cuda()
        inputs, targets = torch.autograd.Variable(inputs), torch.autograd.Variable(targets)
        if fgsm and epsilon > 0.0:
            adv_inputs = fgsm_attack(model, criterion, inputs, targets, epsilon)
