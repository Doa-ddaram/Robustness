...
def stdp_conv2d_single_step(
    ...
    if in_spike.dim() == 3:
        in_spike = in_spike.unsqueeze(1)
    if out_spike.dim() == 3:
        out_spike = out_spike.unsqueeze(1)
    if conv.padding == (0, 0):
        pass
    else:
        pH = conv.padding[0]
    ...
    if trace_pre is None:
        ...
        trace_pre = trace_pre.to(in_spike.device) 

    if trace_post is None:
        trace_post = torch.zeros_like(
            out_spike, device=in_spike.device, dtype=in_spike.dtype)
    else:
        trace_post = trace_post.to(in_spike.device)
    out_spike = out_spike.to(in_spike.device)
    trace_pre = trace_pre - trace_pre / tau_pre + in_spike
    trace_post = trace_post - trace_post / tau_post + out_spike
    delta_w = torch.zeros_like(conv.weight.data, device=in_spike.device)
    for h in range(conv.weight.shape[2]):
        for w in range(conv.weight.shape[3]):
            ...
            weight = conv.weight.data[:, :, h, w].to(in_spike.device)   # shape = [batch_size_out, C_in]
            ...
            delta_w_post = f_post(weight) * \
                           ...
                           .permute([1, 2, 0, 3, 4]).sum(dim = [2, 3, 4])
            delta_w_pre = delta_w_pre.to(in_spike.device)
            delta_w_post = delta_w_post.to(in_spike.device)
...
def stdp_multi_step(
    ...
    delta_w = torch.zeros_like(weight, device=weight.device)
    T = in_spike.shape[0]
...
class STDPLearner(base.MemoryModule):
    ...
    def step(self, on_grad: bool = True, scale: float = 1.):
        ...
        for _ in range(length):
            ...
            print(in_spike.shape, out_spike.shape)
