...
from .. import functional, neuron, layer,surrogate
    ...
    def make_layers(cfg, batch_norm=False, norm_layer=None):
        ...
        for v in cfg:
            if v == 'M':
                ...
                if batch_norm:
                    layers += [conv2d, norm_layer(v), neuron.LIFNode(surrogate_function=surrogate.ATan())]
                else:
                    layers += [conv2d, neuron.LIFNode(surrogate_function=surrogate.ATan())]
                in_channels = v
...
def _spiking_vgg(arch, cfg, batch_norm, pretrained, progress, norm_layer: callable = None, **kwargs):
    if pretrained:
    ...
    if batch_norm:
        ...
        norm_layer = None
    model = SpikingVGG(cfg=cfgs[cfg], batch_norm=batch_norm, norm_layer=norm_layer, **kwargs)
    if pretrained:
...
def spiking_vgg11(pretrained=False, progress=True,**kwargs):
    """
    ...
    return _spiking_vgg('vgg11', 'A', False, pretrained, progress, None, **kwargs)
