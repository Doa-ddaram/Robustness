...
if __name__ == "__main__":
    ...
    if indicate:
        ...
        wandb.init(project=data_set, group=network, config=config, name=data_set + "_" + network)
    ...
    if indicate:
        for i in range(len(loss_acc[0])):
            if attack:
                wandb.log({"attack loss": loss_acc[4], "attack acc": loss_acc[5]}, step=i)
            wandb.log(
                {
